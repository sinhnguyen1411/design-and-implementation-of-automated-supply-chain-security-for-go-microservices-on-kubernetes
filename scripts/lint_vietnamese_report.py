...
BLACKLIST_RULES = [
    ...
    (r"(?i)\blọt lộ\b", "rò rỉ"),
    (r"(?i)\blộ lọt\b", "rò rỉ"),
    (r"(?i)\bchặn oan\b", "gián đoạn do cảnh báo sai lệch"),
    (r"(?i)\bép chạy xanh\b", "bỏ qua kiểm tra an ninh"),
    (r"(?i)\bPipeline cũ\b", "Pipeline truyền thống (ci-service.yml)"),
    (r"(?i)\bcảnh báo giả\b", "cảnh báo sai lệch (False Positives)"),
    (r"(?i)\bpass xanh\b", "phê duyệt bản dựng (Pass)"),
    (r"(?i)\bchạy xanh\b", "vượt qua kiểm tra an ninh"),
    (r"(?i)\bbắt chẹt\b", "rủi ro chi phí bản quyền thương mại leo thang"),
    (r"(?i)\bbắt trúng\b", "nhận diện chính xác"),
    (r"(?i)\bbắt được\b", "nhận diện chính xác"),
    (r"(?i)\bbắn thử\b", "gửi yêu cầu kiểm thử"),
    (r"(?i)\bchỉnh sửa lén\b", "chỉnh sửa trái phép"),
